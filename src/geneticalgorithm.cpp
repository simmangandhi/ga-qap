...
Solution GeneticAlgorithm::run(){
	...
	while(generation < maxgenerations && bestSolution > 0){
		...
		/*Sorts population in increasing order*/
		sort(population.begin(), population.end(), comp);
		if(population[0] >= bestSolution && generation - lastmutation > 50 && generation - this->iterBest > 100 && mutation_type==1){ //Solution converged
		...
		}else if(population[0] >= bestSolution && generation - lastmutation > 50 && generation - this->iterBest > 100 && mutation_type==2){
